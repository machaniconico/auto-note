...
`auto-note support` とGUIの `問い合わせ作成` は、`.auto-note/support` に問い合わせ用Markdownを作成します。標準では診断プレビューを匿名化した状態で埋め込みます。`auto-note support --bundle` とGUIの `問い合わせ一式` は、問い合わせMarkdown、診断レポートZIP、送付前チェックリスト `SUPPORT_SEND_CHECKLIST.txt` を1つのzipにまとめ、manifest/checksumも同梱します。
