...
- `Ctrl+K` コマンド検索
- GUIの日本語フォント優先順を文字が潰れにくい `メイリオ` (`Meiryo`) / `Meiryo UI` 先頭に直し、標準表示の文字サイズ/行高/ボタン余白を1段広げ、Yu/BIZ/MS Gothic系フォントに落ちた場合もGUIスモーク/表示診断で検知できるようにし、`auto-note safe display.lnk` / `auto-note gui --safe-display` / `auto-note-gui.bat --safe-display` で一時的に大きめ表示起動でき、配布ZIPの初回案内にも表示崩れ時の入口を明記する修正
