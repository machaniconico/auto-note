...
- ホームの `購入者送付` 行に `送付証跡` を追加し、送るZIP名、SHA-256先頭、送付文/送付記録の一致状態を同じ場所で確認できる導線
- GUI/CLIの `送付前チェック` と `送付文コピー` でも購入者ZIPが最新配布ZIP由来かを照合し、古い購入者ZIPなら `DO NOT SEND` として送付を止める判定
- GUIの `購入者ZIP場所` で、送付前チェックと購入者ZIP検証を通過したZIPの保存フォルダを直接開き、添付ファイル選択の迷いを減らす導線
- GUIの `ZIPパスコピー` で、送付前チェックと購入者ZIP検証を通過したZIPの絶対パスだけをコピーし、販売ページや注文管理画面への誤添付を減らす導線
- GUIの `送付情報コピー` で、購入者ZIP名、絶対パス、サイズ、SHA-256、送付文ファイルを一括コピーし、販売ページ公開前の照合値をまとめて確認できる導線
- 購入者送付前チェックレポートの送付関連ファイル欄に最新配布ZIP名を表示し、古い購入者ZIPを見分けやすくする証跡表示
- 販売者向け `送付記録` に送付時点の最新配布ZIP名を残し、注文後の問い合わせや送付内容確認で追跡しやすくする証跡表示
- GUIの `送付記録コピー` で最新の販売者向け納品記録を再コピーし、注文管理や問い合わせ対応へ控えを貼り直しやすくする導線
- 販売者向け `送付記録` に `注文管理コピー欄` を追加し、ZIP名、SHA-256、送付文、最新配布ZIP名、販売者専用メモを販売サイト側の注文メモへ貼りやすくする改善
- GUIの `注文控えコピー` で、販売者向け送付記録のうち注文管理へ貼る短い控え欄だけをコピーし、長い販売者専用記録の貼り過ぎを減らす導線
- CLIの `sales-finalize --order-note` で、GUIが使えない環境でも最新送付記録から注文管理へ貼る短い控え欄だけを出力できる導線
- GUIの `販売確認コピー` とCLIの `sales-launch --latest-confirmation` で、販売ページ確認後の販売者専用証跡を注文/掲載管理へ控え直しやすくする導線
...
- GUIホームの販売準備サマリーに `納品照合` を追加し、最新配布ZIP名、購入者ZIP内の配布ZIP名、一致/要更新/検証NGを同じ行で確認できるようにした改善
...
- `auto-note sales-review` / GUI `最終レビュー` と `auto-note sales-launch` / GUI `販売直前` に最新配布ZIP名を証跡として表示し、購入者向けZIPがどの配布物由来か販売直前に照合しやすくする改善
