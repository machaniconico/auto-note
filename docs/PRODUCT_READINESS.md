...
- ホームの販売準備サマリーで、起動を重くしない軽量判定、販売者残件、販売者情報の次項目、生成物不足/NG件数、配布ZIP/販売素材/掲載キット/販売一式/掲載画像の有無/NG、購入者ZIP有無、送付文有無、送付記録有無、最新の販売前一括チェック状態バッジ、24時間超なら `再実行` に切り替わる鮮度警告、未実行なら `一括実行`、新しい結果があれば `結果表示` に切り替わるボタン、販売向けの次アクションを常時確認でき、`次を実行` から該当GUI操作へ進める。壊れた生成物がある時はREADY扱いにせず、次の一手も作り直しへ向ける。販売者情報の動的ボタンは、未入力/確認中なら `販売者次へ` として設定欄へ、6項目が整った後は `販売素材へ` として素材作成へ進める。`販売準備タイムライン` では販売者情報、配布ZIP、販売素材、掲載画像、掲載キット、販売一式、購入者ZIP/送付文、送付前照合、販売直前、販売前一括チェックの10工程をOK/CHECK/NGで確認できる。生成物がそろった後は送付文、購入者ZIP、チェックリスト、販売証跡JSONを照合する `送付前チェック` と、販売ページ文案まで含めた `最終レビュー` へ進める
