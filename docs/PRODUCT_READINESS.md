...
- GUIは高DPI環境で `メイリオ` / `Meiryo` を優先し、初期ウィンドウ/最小サイズを表示倍率に合わせて広げるため、タブやボタンの日本語が潰れにくい
...
- GUIの `最終レビュー` / `レビュー保存` と `auto-note sales-review` / `auto-note sales-review --project-dir . --report` で、最新販売素材、掲載キットZIP、販売者設定、購入者向け送付文、購入者ZIP、送付記録を販売ページ/決済後メッセージ目線で照合し、販売者用の最終確認証跡を `.auto-note\sales\sales-review-*.txt` に保存できる
