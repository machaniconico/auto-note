...
- ホームの `今日のオペレーション` で、最優先、販売/送付、復旧/安全を同じ行に並べ、NG/確認/OKの状態バッジと短い次アクションで迷わず次へ進める。`要約コピー` で販売前確認や問い合わせ時の状態共有用に同じ3項目を短いテキストとして控えられ、`要約保存` で `.auto-note\reports\home-operation-*.txt` にその時点の運用証跡を残せる
...
- ホームの `直近レポート` で、問い合わせZIP、復旧レポート、診断ZIP、配布ZIP、販売直前チェック、販売確認記録、運用要約、投稿キューなどの最新成果物の状態を確認し、表示、保存場所、パスコピーまでできる
