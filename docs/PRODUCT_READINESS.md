...
- GUIは高DPI環境で `Yu Gothic` / `Meiryo UI` / `Meiryo` を優先し、`Yu Gothic UI` と `Noto Sans JP` はフォールバック扱いにして、標準表示を大きくしすぎない文字サイズ/行高/ボタン余白に調整しているため、タブやボタンの日本語が潰れにくい
