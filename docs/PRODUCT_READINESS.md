...
- GitHub Actions の `CI` で、Windows上のPython構文チェック、全unittest、製品品質ゲート、隠しGUIランチャーVBS構文チェック、`scripts\smoke-install.ps1` によるインストール/更新/アンインストールスモーク、GUI smokeをpush/PRごとに確認できる
...
- 自動テストのさらなる拡充と、GitHub Actions上での出荷ZIP系スモーク拡張
