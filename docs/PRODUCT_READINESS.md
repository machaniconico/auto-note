...
- GUIは高DPI環境で `Yu Gothic UI` / `Yu Gothic` / `Meiryo UI` を優先し、`Noto Sans JP` はフォールバック扱いにして、初期ウィンドウ/最小サイズ、行高、ボタン余白を表示倍率に合わせて広げるため、タブやボタンの日本語が潰れにくい
...
- ホームの販売準備サマリーで、起動を重くしない軽量判定、販売者残件、販売者情報の次項目、生成物不足、購入者ZIP有無、送付文有無、送付記録有無、最新の販売前一括チェック状態バッジ、販売向けの次アクションを常時確認でき、`次を実行` から該当GUI操作へ進める。販売者情報の動的ボタンは、未入力/確認中なら `販売者次へ` として設定欄へ、6項目が整った後は `販売素材へ` として素材作成へ進める。`販売準備タイムライン` では販売者情報、配布ZIP、販売素材、掲載画像、掲載キット、販売一式、購入者ZIP/送付文、送付前照合、販売直前、販売前一括チェックの9工程をOK/CHECK/NGで確認できる。生成物がそろった後は送付文、購入者ZIP、チェックリスト、販売証跡JSONを照合する `送付前チェック` と、販売ページ文案まで含めた `最終レビュー` へ進める
- ホームの `直近レポート` で、問い合わせZIP、復旧レポート、診断ZIP、配布ZIP、販売直前チェック、販売確認記録、投稿キューなどの最新成果物の状態を確認し、表示、保存場所、パスコピーまでできる
...
- `scripts\smoke-sales-delivery.ps1` で、クリーンな一時コピー上の販売一括作成、購入者向けZIP検証、送付前チェック保存、販売者向け送付記録作成、購入者送付の照合値付き販売直前チェックリスト保存、販売確認記録の保存を確認できる
