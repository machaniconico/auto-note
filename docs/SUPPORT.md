...
- 文字がつぶれる、ボタン文字が読みにくい、画面倍率が合わない場合は、GUIの `診断` タブまたはコマンド検索の `表示診断` を実行できるか。必要なら `表示診断コピー` で可読性判定をそのまま共有できます。GUIの `問い合わせ一式` で作成したZIPには `DISPLAY_DIAGNOSTICS.txt` として同梱されます
