...
- 文字がつぶれる、ボタン文字が読みにくい、画面倍率が合わない場合は、GUIの `診断` タブまたはコマンド検索の `表示診断` を実行できるか。必要なら `表示診断コピー` で、表示フォント、倍率、可読性判定、潰れやすいフォント判定をそのまま共有できます。GUIの `問い合わせ一式` で作成したZIPには `DISPLAY_DIAGNOSTICS.txt` として同梱されます
...
1. GUIの `問い合わせ一式` で作成したzip（トップ階層の `GUI_LOG_SUMMARY.txt` に最新GUIログ要約、`DISPLAY_DIAGNOSTICS.txt` に表示/フォント/倍率診断が入り、GUIの `ZIPログ要約` / `ZIP表示診断` でも確認できます）
