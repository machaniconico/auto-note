...
- 文字がつぶれる、ボタン文字が読みにくい、画面倍率が合わない場合は、GUIの `診断` タブまたはコマンド検索の `表示診断` を実行できるか。必要なら `表示診断コピー` で、表示フォント、倍率、可読性判定、ボタン文字の収まり、潰れやすいフォント判定をそのまま共有できます。GUIの `問い合わせ一式` で作成したZIPには `DISPLAY_DIAGNOSTICS.txt` として同梱されます
...
- note.com へのログインは普段使っているブラウザでできるか。Googleログインなどで「このブラウザまたはアプリは安全ではない可能性があります」と表示される場合は、GUIの `ログイン安全ガイド`、または `auto-note login --default-browser` を使い、投稿は `投稿ヘルパー` で貼り付けます
- 記事ファイルが `articles` フォルダにあるか

## ユーザーに送ってもらうもの

1. GUIの `問い合わせ一式` で作成したzip（トップ階層の `GUI_LOG_SUMMARY.txt` に最新GUIログ要約、`DISPLAY_DIAGNOSTICS.txt` に表示/フォント/倍率/ボタン文字収まり診断が入り、GUIの `ZIPログ要約` / `ZIP表示診断` でも確認できます）
...
`auto-note support --bundle` またはGUIの `問い合わせ一式` は、問い合わせMarkdown、最新GUIログ要約 `GUI_LOG_SUMMARY.txt`、診断レポートZIP、送付前チェックリスト `SUPPORT_SEND_CHECKLIST.txt` を1つのzipにまとめます。GUIから作成した問い合わせ一式には、文字つぶれやボタン文字の収まり、DPI/フォント問題を追うための `DISPLAY_DIAGNOSTICS.txt` も同梱されます。作成直後にmanifest/checksum検証とプライバシー監査を行い、問題がある場合はGUIではファイルを開かずに結果を表示します。問い合わせ一式ZIPには `SUPPORT_BUNDLE_MANIFEST.json` と `CHECKSUMS.txt` も入り、GUIの `一式ZIP検証` または `auto-note support --verify <zip>` で検証できます。送付前は `SUPPORT_SEND_CHECKLIST.txt` の送付判断を確認し、検証NGのZIPは送らず作り直します。サポートから明示されない限り、この問い合わせ一式ZIPだけを送り、`.auto-note` フォルダ、記事、販売一式ZIP、購入者ZIP、注文情報は添付しません。

初回チェック/受入チェックでは、問い合わせ一式ZIPが24時間を超えて古い場合に `WARN` になります。`auto-note support --verify <zip>`、GUIの `一式ZIP検証`、診断プレビューの `maintenance-summary.txt` でも鮮度と経過時間を確認できます。問い合わせ前は最新のGUIログ、表示診断、診断ZIPを含めるため、送付直前にGUIの `問い合わせ一式` で作り直してください。
