...
def _build_start_here() -> str:
    return (
        ...
        "- auto-note.lnk を開きます。文字や行高が潰れる場合は auto-note safe display.lnk を開きます。\n"
        "- ホームまたは初回タブの 受入チェック で NG がないことを確認します。\n"
        "- 見え方の相談が必要な場合は 表示診断コピー と 問い合わせ一式 を作ります。\n"
        "\n"
        "1. このzipを好きな場所に展開します。\n"
        "2. shortcuts\\install-auto-note.bat をダブルクリックします。\n"
        "3. デスクトップまたはスタートメニューの auto-note を開きます。\n"
        "4. 文字や行高が潰れて見える場合は auto-note safe display.lnk を開きます。\n"
        "5. GUIのセットアップウィザードで基本設定を確認します。\n"
        "6. 起動後に不足が出る場合は GUI の 自動修復、または auto-note repair --project-dir . --apply を使います。\n"
        "7. 起動、ログイン、問い合わせ前の切り分けは GUI の トラブル診断、または auto-note troubleshoot --project-dir . を使います。\n"
        "8. 初めて触る場合は GUI の スターター一式 を押します。\n"
        "9. スターターを片付ける場合は GUI の スターター整理 で候補を確認します。\n"
        "10. 自分の記事を作る場合は GUI の 新規記事 を押します。\n"
        "11. note に投稿する時は 投稿ヘルパー を開き、普段のブラウザへ貼り付けます。\n"
        "12. 販売/配布前の確認は GUI の 出荷前チェック または 出荷ZIP作成 を使います。\n"
        "13. 購入者への納品確認は BUYER_ACCEPTANCE_CHECKLIST.txt と auto-note acceptance --project-dir . --full を使います。\n"
        "\n"
        "インストールせず試す場合:\n"
        "- auto-note-gui.bat を直接開きます。\n"
        "- 文字や行高が潰れて見える場合は auto-note-gui.bat --safe-display を使います。\n"
        "\n"
        "困った時:\n"
        "- docs\\INSTALL.md を確認します。\n"
        "- docs\\SUPPORT.md を確認します。\n"
        "- GUIのヘルプで 問い合わせ一式 を作成できます。\n"
        "- 文字が読みづらい場合は 表示診断コピー を添えると、フォント、倍率、ボタン文字の収まりを確認しやすくなります。\n"
        "- CLIでは auto-note support --project-dir . --bundle を使います。\n"
        "- GUIが起動しない場合は auto-note-gui.bat を開くとエラー内容を確認できます。\n"
        "- GUIは開くが文字が読みにくい場合は auto-note safe display.lnk または auto-note-gui.bat --safe-display を使います。\n"
        "\n"
        "配布内容の確認:\n"
...
def _build_first_run_checklist() -> str:
    return (
        ...
        "[ ] 3. 文字や行高が潰れる場合は auto-note safe display.lnk で開けた\n"
        "[ ] 4. GUIの セットアップ で既定タグと投稿ヘルパー設定を確認した\n"
        "[ ] 5. GUIの 自動修復 で基本フォルダ/設定の修復導線を確認した\n"
        "[ ] 6. GUIの 初回チェック と セルフテスト保存 を確認した\n"
        "[ ] 7. スターター一式 でサンプル記事、予定、アイデアを作った\n"
        "[ ] 8. 全体チェック と 記事レビュー を確認した\n"
        "[ ] 9. 投稿ヘルパー を開き、タイトル/本文/タグをコピーできることを確認した\n"
        "[ ] 10. GUIの ログイン安全ガイド を確認し、普段使うブラウザで note.com にログインした\n"
        "[ ] 11. バックアップ作成 を実行した\n"
        "[ ] 12. 困った時に ヘルプ > 問い合わせ一式 を作れることを確認した\n"
        "\n"
        "CLIで同じ確認をする場合:\n"
        ...
        "- 文字や行高が潰れて見える場合は auto-note safe display.lnk または auto-note-gui.bat --safe-display で開きます。\n"
        "- GUIが開く場合は 表示診断コピー で見え方の情報をコピーできます。\n"
        "- .auto-note\\gui-error.log があればサポートへ共有できます。\n"
        "- docs\\SUPPORT.md の手順で問い合わせ一式を作成します。\n"
        "\n"
        "投稿で迷った場合:\n"
        "- GUIの ログイン安全ガイド で、安全ではない可能性がある表示時の回避手順を確認します。\n"
...
def _build_buyer_acceptance_checklist() -> str:
    return (
        ...
        "\n"
        "最初の3分の合格ライン:\n"
        "- auto-note.lnk または auto-note safe display.lnk でGUIを開ける\n"
        "- 受入チェックが BLOCKED ではない\n"
        "- 投稿ヘルパーから普段使うブラウザへコピーできる\n"
        "\n"
        "購入者が受け取り後に確認すること:\n"
        "\n"
        "[ ] 1. auto-note を開けた\n"
        "[ ] 2. 文字や行高が潰れる場合は auto-note safe display.lnk で開けた\n"
        "[ ] 3. セットアップ画面で既定タグと投稿ヘルパー設定を確認した\n"
        "[ ] 4. 受入チェックを実行し、NGがないことを確認した\n"
        "[ ] 5. スターター一式でサンプル記事、予定、アイデアを作成できた\n"
        "[ ] 6. 投稿キュー、運用サマリー、予定タブを確認できた\n"
        "[ ] 7. 投稿ヘルパーを開き、タイトル/本文/タグをコピーできた\n"
        "[ ] 8. GUIの ログイン安全ガイド を確認し、普段使うブラウザでnote.comにログインできた\n"
        "[ ] 9. バックアップ作成を実行できた\n"
        "[ ] 10. 見え方の相談時に 表示診断コピー を使えることを確認した\n"
        "[ ] 11. 困った時にヘルプから問い合わせ一式を作れる場所を確認した\n"
        "\n"
        "CLIで確認する場合:\n"
        "- auto-note acceptance --project-dir . --full\n"
        "- auto-note troubleshoot --project-dir .\n"
        ...
        "- GUI smoke が OK\n"
        "- Privacy audit が OK\n"
        "- 表示診断で readability と button label fit が OK\n"
        "- noteログインは ログイン安全ガイド を確認したうえで普段使うブラウザで行える\n"
    )
...
def _build_release_summary(project_dir: Path, created_at: datetime) -> str:
    ...
    return (
        ...
        "- auto-note safe display.lnk if text looks cramped after installing\n"
