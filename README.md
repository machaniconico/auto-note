...
GUIは9つのタブに分かれています。ホームには作業進行レーン、アクションプランの最優先項目、上位の優先アクション一覧、直近レポートが表示され、`実行` / `選択を実行` でその操作へ直接進めます。作業進行レーンの各工程の `開く` から、初回、記事、仕上げ、投稿、販売、サポートの該当画面へ直接移動できます。ヘッダーの `コマンド` または `Ctrl+K` からも `作業進行: 初回` などで各工程へ移動でき、検索結果の件数と一致するコマンドがない時の案内も表示します。必要な時は選択中のCLIもコピーでき、直近レポートでは問い合わせZIP、復旧レポート、診断ZIP、配布ZIP、投稿キューなどの最新ファイルの状態を見ながら、表示、保存場所の確認、パスコピーができます。
