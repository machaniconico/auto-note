...
- `復旧セット` はGUIの診断、ヘルプ、コマンド検索から実行でき、安全な基本修復、再診断、必要時の問い合わせ一式ZIP作成までを1回で行います。GUIが起動しない時は `auto-note recovery-kit --project-dir .` でも同じ復旧セットを実行できます。記事や古い生成物は変更/削除しません。
