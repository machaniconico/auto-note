...
GUIは9つのタブに分かれています。ホームにはアクションプランの最優先項目、上位の優先アクション一覧、直近レポートが表示され、`実行` / `選択を実行` でその操作へ直接進めます。必要な時は選択中のCLIもコピーでき、直近レポートでは問い合わせZIP、復旧レポート、診断ZIP、配布ZIP、投稿キューなどの最新ファイルの状態を見ながら、表示、保存場所の確認、パスコピーができます。

- `ホーム`: 今日の状態、優先アクション一覧、状態表示とパスコピー付きの直近レポート、送付記録有無まで見える販売準備サマリー、スターター一式、運用サマリー、次の作業、公開予定の概要
