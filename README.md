...
- `auto-note sales-launch --project-dir . --confirm-preview --note "checked"` またはGUIの `販売確認記録` で、販売ページのプレビュー/テスト購入相当を目視確認した後の販売者専用証跡を `.auto-note\sales\sales-launch-confirmation-*.txt` に保存できます。この記録は購入者向けZIPや問い合わせ一式には含めず、販売者だけが保管します。
