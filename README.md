...
- `scripts\check-release.ps1` で、Python構文チェック、全unittest、製品品質ゲート、VBSランチャー構文チェック、GUI smokeを手元で一括確認できます。販売直前は `-Full` を付けると、プライバシー監査、販売準備、preflight、インストール/アンインストールスモーク、販売一括/購入者送付スモークまで確認できます。
- `scripts\smoke-install.ps1` で、インストール/アンインストール導線だけを一時フォルダで検証できます。`scripts\smoke-sales-delivery.ps1` では、クリーンな一時コピー上の販売一括作成、購入者向けZIP検証、送付前チェック保存、販売者向け送付記録作成を確認できます。
