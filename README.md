...
GUIは9つのタブに分かれています。ホームにはアクションプランの最優先項目、上位の優先アクション一覧、直近レポートが表示され、`実行` / `選択を実行` でその操作へ直接進めます。必要な時は選択中のCLIもコピーでき、直近レポートでは問い合わせZIP、復旧レポート、診断ZIP、配布ZIP、投稿キューなどの最新ファイルを表示または保存場所から確認できます。

- `ホーム`: 今日の状態、優先アクション一覧、直近レポート、送付記録有無まで見える販売準備サマリー、スターター一式、運用サマリー、次の作業、公開予定の概要
