...
- `auto-note sales-review` で、最新販売素材、掲載キットZIP、販売者設定、購入者向け送付文、購入者ZIP、送付記録を販売ページ/決済後メッセージ目線で照合できます。GUIの `最終レビュー` で表示し、`auto-note sales-review --project-dir . --report` またはGUIの `レビュー保存` で `.auto-note\sales\sales-review-*.txt` に販売者用の最終確認証跡を保存できます。
