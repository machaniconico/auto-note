...
- `scripts\smoke-install.ps1` で、インストール/アンインストール導線だけを一時フォルダで検証できます。`scripts\smoke-sales-delivery.ps1` では、クリーンな一時コピー上の販売一括作成、購入者向けZIP検証、送付前チェック保存、販売者向け送付記録作成、購入者送付の照合値付き販売直前チェックリスト保存を確認できます。
