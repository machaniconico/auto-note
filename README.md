...
GUIは9つのタブに分かれています。ホームには作業進行レーン、アクションプランの最優先項目、上位の優先アクション一覧、直近レポートが表示され、`実行` / `選択を実行` でその操作へ直接進めます。作業進行レーンの各工程の `開く` から、初回、記事、仕上げ、投稿、販売、サポートの該当画面へ直接移動できます。ヘッダーの `コマンド` または `Ctrl+K` からも `作業進行: 初回` などで各工程へ移動でき、検索結果の件数と一致するコマンドがない時の案内も表示します。`投稿 準備` のようにスペース区切りの複数語でも探せ、上下キーで候補を選び、Enterで実行できます。必要な時は選択中のCLIもコピーでき、直近レポートでは問い合わせZIP、復旧レポート、診断ZIP、配布ZIP、投稿キューなどの最新ファイルの状態を見ながら、表示、保存場所の確認、パスコピーができます。

- `ホーム`: 今日の状態、初回/記事/仕上げ/投稿/販売/サポートを横断する作業進行、優先アクション一覧、状態表示とパスコピー付きの直近レポート、送付記録有無まで見える販売準備サマリー、スターター一式、運用サマリー、次の作業、公開予定の概要
