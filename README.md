...
基本は `auto-note.lnk` だけ使えば大丈夫です。ダブルクリックすると、スターター一式、記事作成、フォルダを開く、公開前チェック、改善プラン、投稿準備、投稿キュー、運用サマリー、ダッシュボード、投稿ヘルパー、noteログイン、予定管理、予定ICS出力、アイデア箱、復旧セット、自動修復、トラブル診断、受入チェック、販売準備、販売用一式、販売直前チェック、販売前一括チェックを一画面で操作できます。初回は `セットアップ` と `初回チェック`、購入/納品後の確認は `受入チェック`、起動や設定で詰まった時は `復旧セット`、販売前の最終確認は `販売準備`、販売直前の目視証跡は `販売直前` / `直前保存` / `販売確認記録` / `販売確認コピー`、重い最終ゲートは `販売前一括チェック`、迷った時は `コマンド` または `Ctrl+K` から主要操作を検索できます。

トップ直下の起動ファイルはこの4つです。

- `auto-note.lnk`: アイコン付きGUIショートカット
- `auto-note safe display.lnk`: 文字や行高が潰れる環境向けの大きめ表示ショートカット
...
GUI操作中にエラーが出た場合も、画面には `GUIログ表示`、`復旧セット`、`問い合わせ一式` の次アクションを表示します。解決後は `GUIログクリア` で確認済みログを退避し、復旧ステータスをOKに戻せます。`Ctrl+K` のコマンド検索から同じ操作をすぐ開けます。
ホームの `復旧ステータス` でも最新GUIログの有無、更新時刻、サイズを確認でき、`GUIログ表示`、`ログクリア`、`復旧セット`、`問い合わせ一式` へ直接進めます。

配布ZIPから使う場合は、展開後に `shortcuts\install-auto-note.bat` を実行すると、管理者権限なしで `%LOCALAPPDATA%\auto-note` に配置し、デスクトップとスタートメニューに `auto-note` と `auto-note safe display` ショートカットを作成します。インストールせずに試す場合は `auto-note-gui.bat` を直接開けます。アンインストールは `shortcuts\uninstall-auto-note.bat` から実行でき、標準では記事と設定を残します。

GUIは9つのタブに分かれています。ホームには準備度、次の一手、初回/復旧、販売/送付を1行で見られるコンパクト概要、今日のオペレーション（最優先/販売・送付/復旧・安全）、作業進行レーン、初回セットアップのスコアと次項目、アクションプランの最優先項目、上位の優先アクション一覧、直近レポートが表示され、`実行` / `選択を実行` でその操作へ直接進めます。今日のオペレーションは `要約コピー` で、販売前確認や問い合わせ時に最優先、販売/送付、復旧/安全の状態を短く控えられ、`要約保存` で `.auto-note\reports\home-operation-*.txt` に証跡として残せます。作業進行レーンの各工程の `開く` から、初回、記事、仕上げ、投稿、販売、サポートの該当画面へ直接移動できます。`初回セットアップ` の `初回を開く` では、初回チェックの未完了項目を選んだ状態で確認できます。ホームの `次の作業` は `操作検索` で `販売 保存` や `送付 ZIP` のように絞り込めます。ホームの `販売準備` には販売者情報の次に直す項目、生成物NG件数、生成物更新状態、配布ZIP/販売素材/掲載キット/販売一式の有無/NG、掲載画像の有無/NG、`購入者送付` の状態、最新の `販売前一括チェック` の状態バッジと未実行なら `一括実行`、結果があれば `結果表示`、24時間超なら `再実行` に切り替わるボタン、鮮度警告、`販売準備タイムライン` が表示され、販売者情報、配布ZIP、販売素材、掲載画像、掲載キット、販売一式、購入者ZIP/送付文、送付前照合、販売直前、販売前一括チェックの10工程を確認できます。販売者情報が未完了の時は `販売者次へ`、整った後は `販売素材へ` に変わるボタンから次工程へ進めます。壊れた生成物や最新配布ZIPとずれた販売用一式/購入者ZIPがある時はREADY扱いにせず、次の一手も作り直しへ向けます。`納品照合` では最新配布ZIP名、購入者ZIP内の配布ZIP名、一致/要更新/検証NGを同じ行で確認できます。`送付証跡` では購入者ZIP名、SHA-256先頭、送付文一致/不一致、送付記録一致/不一致を同じ行で確認できます。購入者ZIP、送付文、送付記録、購入者ZIPが最新配布ZIP由来か、送付文と最新ZIP名/SHA-256の照合結果、送付記録と最新ZIP/送付文の照合結果も確認でき、状態に応じた購入者送付ボタンから、ZIP作成、ZIP更新、送付文作成、送付記録、最終レビュー、販売直前チェックへ直接進めます。ヘッダーの `コマンド` または `Ctrl+K` からも `作業進行: 初回` などで各工程へ移動でき、検索結果の件数と一致するコマンドがない時の案内も表示します。`投稿 準備` のようにスペース区切りの複数語でも探せ、上下キーで候補を選び、Enterで実行できます。必要な時は選択中のCLIもコピーでき、直近レポートでは問い合わせZIP、復旧レポート、診断ZIP、配布ZIP、掲載キットZIP、購入者ZIP、購入者送付文、送付記録、販売直前チェック、販売確認記録、販売前一括チェック、運用要約、投稿キューなどの最新ファイルの状態を見ながら、表示、保存場所の確認、パスコピーができます。

- `ホーム`: 今日の状態、今日のオペレーション、初回/記事/仕上げ/投稿/販売/サポートを横断する作業進行、初回セットアップのスコアと次項目、優先アクション一覧、購入者送付ファイルまで開ける直近レポート、販売者情報の次項目と動的ボタン、生成物不足/NG件数/更新状態、配布ZIP/販売素材/掲載キット/販売一式/掲載画像の有無/NG、購入者ZIP/送付文/送付記録/最新配布ZIP一致、納品照合、送付証跡と最新の販売前一括チェックまで見える販売準備サマリー、10工程の販売準備タイムライン、スターター一式、運用サマリー、操作検索つきの次の作業、公開予定の概要
...
- `診断`: 初回チェック、受入チェック、販売準備、販売用一式、販売直前チェック、販売前一括チェック、運用サマリー、環境チェック、復旧セット、自動修復、トラブル診断、出荷前チェック、出荷ZIP作成、ログ表示、GUIログ場所、バックアップ、診断レポート、診断ZIP検証、診断ZIP場所、診断ZIPパス、配布ZIP作成
...
- GUIは詰まりにくい `標準` 表示で起動し、日本語UIに向いた `Yu Gothic` / `Meiryo UI` / `Meiryo` 優先、DPIぼけを抑える表示設定、軽めの見出し/ステータス文字にしています。`Yu Gothic UI` と `Noto Sans JP` はフォールバックとして残しつつ、Tk/Windowsで文字が詰まって見える場合は表示診断で注意できるようにしています。標準表示は大きくしすぎず、ボタンやタブの漢字が潰れにくいフォント・余白・自動幅のヘッダー操作にしています。文字や行高が小さく見える場合は、ヘッダーの `表示` で `ゆったり` または `大きめ` を選ぶとすぐ拡大できます。GUIを開く前から見づらい場合は `auto-note safe display.lnk`、`auto-note gui --project-dir . --safe-display`、または `auto-note-gui.bat --safe-display` で一時的に大きめ表示で起動できます。`Ctrl+K` から `表示サイズ: 大きめ` を実行することもできます。表示やウィンドウ位置が扱いにくい時はヘッダーの `リセット` または `Ctrl+K` の `表示リセット` で既定の見やすい状態に戻せます。細かく選びたい場合は `設定` の `表示サイズ` で `標準` / `ゆったり` / `大きめ` を選んで保存します。見え方の相談時は `診断` の `表示診断`、または `Ctrl+K` の `表示診断` / `表示診断コピー` で指定フォント、実際の表示フォント、倍率、画面サイズ、可読性判定、ボタン文字の収まり、潰れやすいフォント判定を確認・コピーできます。
...
- GUI起動ログと操作中エラーは `.auto-note\gui-error.log` に残ります。GUIの `GUIログ表示` / `GUIログコピー` / `GUIログ場所` から確認、コピー、保存フォルダ表示ができます。解決後は `GUIログクリア` で `gui-error-cleared-*.log` に退避し、現在の復旧ステータスをOKに戻せます。
- GUIの `問い合わせ一式` で作成したZIPに含まれる表示診断は、ヘルプの `ZIP表示診断` から送付前に確認できます。
- 初回チェック/受入チェックでは、問い合わせ一式ZIPが24時間を超えて古い場合に `WARN` として表示し、送付直前の作り直しを促します。`auto-note support --verify <zip>` と診断プレビュー/保守サマリーでも鮮度と経過時間を確認できます。
- インストール/更新時は `.auto-note\install-info.json` に記録が残り、既存記事や設定がある場合は `.auto-note\install-backups` に更新前バックアップが作られます。`auto-note version --project-dir .` と `auto-note diagnose --project-dir .` では、`install-info.json` の破損や、記録された更新前バックアップの参照切れもNGとして確認できます。
- `auto-note diagnose --project-dir .` では、インストール/アンインストール補助ファイルと `auto-note safe display.lnk` の有無も確認できるため、配布ZIPの展開漏れやインストール済みコピーの欠品を問い合わせ前に切り分けられます。
...
- `auto-note repair` で基本フォルダ/設定/アイデア保存の修復プレビュー、壊れた `install-info.json` の退避、プライバシー監査NG生成物候補を確認できます。`--apply` を付けると基本修復と壊れたインストール記録の退避を実行します。削除系は `--cleanup-privacy` や `--cleanup-old` を明示した時だけ対象になります。配布ZIPまで整理候補に含める場合は `--include-releases` も必要です。GUIの診断、ヘルプ、コマンド検索の `自動修復` からも実行できます。
- `auto-note troubleshoot` で、起動ログ、セットアップ、インストール記録、noteログイン詰まり、最新生成物のプライバシー監査、危険生成物候補、最新配布ZIPをまとめて確認できます。プライバシーNG生成物の整理候補は通常配布ZIPを除外し、配布ZIPまで削除前確認に含める場合は `--include-releases` を付けます。`install-info.json` の破損や更新前バックアップの参照切れがある場合は、再インストール/更新や現状バックアップ作成への次アクションも表示します。GUIのホーム、診断、ヘルプ、コマンド検索の `トラブル診断` からも実行できます。
- `auto-note first-run` で、購入/導入直後に見るべきセットアップ、表示の読みやすさ、セルフテスト保存、最初の記事、投稿ヘルパー、バックアップ、問い合わせ一式を順番に確認できます。GUIの `初回` タブでは `要対応だけ` でWARN/NGだけに絞り込めます。`--gui-smoke` を付けると、文字の収まりとボタン文字の収まりも確認し、問題がある場合は `auto-note safe display.lnk`、`表示 > 大きめ`、`表示診断コピー` へ誘導します。記事の仕上げや販売者情報など投稿/販売前のTODOはINFOとして見せ、初回起動確認のWARNと分けて扱います。
- `auto-note acceptance` で、購入/納品後の受入チェックを1枚のレポートにまとめられます。初回チェック、セルフテスト、トラブル診断、投稿ヘルパー、GUI初期化、表示の読みやすさ、問い合わせ一式を確認し、`--report` で `.auto-note\reports\acceptance-*.txt` に保存できます。`--full` を付けると、基本セットアップ作成、GUI初期化、表示可読性確認、投稿ヘルパーHTML生成、レポート保存までまとめて実行します。記事本文の改善提案は投稿前確認として残しつつ、GUI起動や配布物受入のブロッカーとは分けます。GUIのホーム、初回、診断、ヘルプ、コマンド検索の `受入チェック` / `受入保存` / `受入フル保存` からも実行できます。
...
- `auto-note commercial-readiness` で、販売前に見るべき配布ZIP、プライバシー監査、受入チェック証跡、販売者プロフィール、販売文書、利用条件/商用方針、販売最終確認、サポート連絡先、インストール導線を1枚にまとめられます。出力には `RC target / 販売RC目途`、`RC path / 残り作業`、`RC checkpoint / 今回の目途` が入り、BLOCKEDでも「まずどこまで終えれば再判定できるか」を確認できます。`--report` で `.auto-note\reports\commercial-readiness-*.txt` に保存でき、GUIのホーム、初回、診断、ヘルプ、コマンド検索の `販売準備` / `販売準備保存` からも実行できます。
...
- `auto-note sales-review` で、最新販売素材、最新配布ZIP、掲載キットZIP、販売者設定、購入者向け送付文、購入者ZIP、送付記録を販売ページ/決済後メッセージ目線で照合できます。GUIの `最終レビュー` で表示し、`auto-note sales-review --project-dir . --report` またはGUIの `レビュー保存` で `.auto-note\sales\sales-review-*.txt` に販売者用の最終確認証跡を保存できます。
- `auto-note sales-launch` で、最終レビュー後に販売ページ公開直前の掲載キットZIP、決済後メッセージ、添付ZIP、返金/サポート表示、販売者専用証跡をまとめて確認できます。最新配布ZIP名、購入者向けZIPのファイル名、サイズ、SHA-256、送付文ファイルを照合値として表示し、掲載キットZIPは販売ページ作成用で購入者向け添付欄に入れないことも確認します。販売ページURLから note / BOOTH / Gumroad / STORES / 汎用販売ページ向けの目視項目も出し分けます。GUIの `販売直前` で表示し、`auto-note sales-launch --project-dir . --report` またはGUIの `直前保存` で `.auto-note\sales\sales-launch-checklist-*.txt` に販売者専用の目視チェックリストを保存できます。このチェックリストは購入者向けZIPや問い合わせ一式には添付しません。
- `auto-note sales-launch --project-dir . --confirm-preview --note "checkout preview checked: <buyer ZIP name> / <full SHA-256>"` またはGUIの `販売確認記録` で、販売ページのプレビュー/テスト購入相当を目視確認した後の販売者専用証跡を `.auto-note\sales\sales-launch-confirmation-*.txt` に保存できます。確認記録には、確認した画面、最新の購入者ZIP名、完全なSHA-256、プレビュー/テスト購入結果のメモが必要で、メモ内のZIP名/SHA-256が最新の購入者向けZIPと一致しない場合は保存しません。GUIの `販売確認コピー`、またはCLIの `auto-note sales-launch --project-dir . --latest-confirmation` で最新の確認記録だけを再表示/コピーできます。この記録は購入者向けZIPや問い合わせ一式には含めず、販売者だけが保管します。
- GUIの `販売前一括チェック` で `scripts\check-release.ps1 -Full` をバックグラウンド実行し、構文チェック、全unittest、製品品質ゲート、VBSランチャー、GUI smoke、GUI safe display smoke、プライバシー監査、販売準備、preflight、インストール/アンインストールスモーク、販売納品スモークをまとめて確認できます。結果は `.auto-note\reports\release-check-*.txt` に保存され、ホームの販売準備サマリーでは状態バッジ付きで、24時間を超えたOKは再実行推奨として表示し、同じボタンも `再実行` に切り替わります。未実行なら `一括実行`、新しい結果があれば `結果表示` から最新結果を開けます。`直近レポート` では状態、表示、保存場所の確認、パスコピー付きで確認できます。
...
- `auto-note sales-screenshots` で、販売ページ掲載用の5枚のSVG画像、キャプションMarkdown、HTMLプレビューを `.auto-note\sales\screenshots\auto-note-sales-screenshots-*` に生成できます。`sales-screenshots --project-dir . --verify <folder>` で不足ファイルや壊れたSVGを検証でき、GUIの `掲載画像作成` / `掲載画像検証` からも実行できます。作成後は `index.html` を開いて、説明文と画像の流れを販売ページ掲載前に確認できます。
- `auto-note sales-listing` で、販売ページへ貼る `SALES_MATERIALS.md`、`images\*.svg`、`SCREENSHOT_CAPTIONS.md`、`index.html`、`LISTING_UPLOAD_CHECKLIST.txt`、`SALES_LISTING_MANIFEST.json`、`CHECKSUMS.txt` を `.auto-note\sales\listing-kits\auto-note-sales-listing-kit-*` にまとめ、同時に `.auto-note\sales\auto-note-sales-listing-kit-*.zip` を作成できます。`sales-listing --project-dir . --verify <folder-or-zip> --strict` で販売者情報の未設定、壊れた画像、checksum不一致、購入者向け納品物の混入を検証でき、GUIの `掲載キット作成` / `掲載キット検証` からも実行できます。このZIPは販売者が販売ページ掲載時に使う専用キットで、購入者へ送る納品ZIPではありません。
- `auto-note sales-handoff` で、最新の配布ZIP、販売素材Markdown、販売ページ掲載画像パック、販売準備レポート、プライバシー監査、配布ZIP検証結果、購入者向け納品文、購入者の最初の10分、購入者へ送るもの/販売者が保管するものを分けた納品チェックリスト、販売者向け納品記録テンプレ、販売者最終チェックリスト、サポート返信テンプレを1つの販売用一式ZIPにまとめられます。GUIの診断、ヘルプ、コマンド検索の `販売一式作成` / `販売一式検証` からも実行できます。販売者はこのZIPを証跡として保管し、`sales_screenshots/index.html` と `sales_screenshots/SCREENSHOT_CAPTIONS.md` で掲載画像とキャプションも確認できます。GUIの `購入者ZIP抽出` または `sales-handoff --project-dir . --extract-buyer <販売一式ZIP>` で、購入者へ送る配布ZIP、`START_HERE_FOR_BUYER.txt`、納品メモ、購入者向けサポートガイド、記入式の `BUYER_SUPPORT_REQUEST.txt`、`BUYER_DELIVERY_MANIFEST.json`、`SHA256SUMS.txt` だけを別フォルダへ取り出し、同時に `auto-note-buyer-delivery-*.zip` を作成できます。GUIの `問い合わせ票` で最新の購入者向け抽出フォルダにある `BUYER_SUPPORT_REQUEST.txt` を直接開けます。GUIの `購入者ZIP検証`、`sales-handoff --project-dir . --verify-buyer <抽出フォルダ>`、`sales-handoff --project-dir . --verify-buyer-package <購入者向けZIP>` で、送付前に余計なファイル混入、配布ZIP破損、最初に読むメモ不足、問い合わせ票不足、manifest不一致、チェックサム不一致を確認できます。既存フォルダから作り直す場合は `sales-handoff --project-dir . --package-buyer <抽出フォルダ>` を使います。
- `auto-note sales-finalize` で、新しい配布ZIP、販売者テンプレート、販売素材Markdown、販売ページ掲載画像パック、販売ページ掲載キット、販売用一式ZIP、チェックサム付き購入者向け抽出フォルダ、購入者へそのまま添付できる `auto-note-buyer-delivery-*.zip`、最新ZIP名/サイズ/SHA-256入りの購入者向け送付文、販売ナビレポート、販売者送付前チェックリスト、販売証跡JSONマニフェスト `sales-evidence-manifest-*.json`、受入チェック証跡、診断ZIP、プライバシー監査、最終出荷前チェックを一括作成できます。古い販売一式ZIPが残っていても作り直せます。結果は `.auto-note\sales\sales-finalize-*.txt` に保存され、掲載画像パックは `.auto-note\sales\screenshots\auto-note-sales-screenshots-*`、掲載キットZIPは `.auto-note\sales\auto-note-sales-listing-kit-*.zip` に保存され、販売ナビは `.auto-note\sales\sales-plan-*.txt` にも保存され、購入者向けZIP/掲載キットZIP/販売者証跡ZIP/配布ZIPのサイズとSHA-256も残ります。販売者送付前チェックリストでは、購入者へ送るZIP、販売者が保管するZIP、販売ナビ証跡、掲載画像パック、掲載キットZIP、販売証跡JSON、元の配布ZIPを別添しないこと、通常送ってはいけないファイルを確認できます。掲載キットZIPは販売ページ作成用で、購入者へ送る納品ZIPではありません。GUIの `販売一括作成` からも実行できます。GUIの `送付前チェック` では最新送付文、購入者向けZIP、最新配布ZIP由来か、販売者チェックリスト、販売証跡JSONを照合し、GUIの `送付文コピー` では対応ZIPを検証してからクリップボードへコピーできます。GUIの `送付記録` またはCLIの `sales-finalize --project-dir . --delivery-receipt` では、送付前チェック証跡、送付時点の最新配布ZIP名、注文管理へ控える `seller-delivery-receipt-*.txt` を保存できます。GUIの `送付記録コピー` では最新の送付記録をそのままクリップボードへ戻せます。CLIでは `sales-finalize --project-dir . --send-check --send-check-report` で同じ照合だけを行い、最新配布ZIP名も含む `.auto-note\sales\buyer-send-readiness-*.txt` を送付前証跡として保存できます。編集済みの最新販売者テンプレートを先に取り込む場合は `sales-finalize --project-dir . --apply-latest-template`、GUIでは `テンプレ取込一括` を使います。未入力の販売者情報や受入チェックのWARN/NGは項目名付きで表示され、`--strict` を付けると警告を本番前のNGとして扱えます。
- GUIの `送付記録コピー` の送付記録には `注文管理コピー欄` があり、ZIP名、SHA-256、送付文、最新配布ZIP名を販売サイト側の注文メモへ貼り直せます。GUIの `注文控えコピー` では、その短い欄だけをコピーできるため、販売者専用の長い記録を注文画面へ貼り過ぎるミスを減らせます。CLIだけで作業する場合は `sales-finalize --project-dir . --order-note` で最新送付記録から同じ短い控え欄だけを出力できます。
- GUIの `購入者ZIP場所` では、送付前チェックと購入者向けZIP検証を通過したZIPの保存フォルダを直接開けます。GUIの `ZIPパスコピー` では、同じガードを通過したZIPの絶対パスだけをクリップボードへコピーできます。GUIの `送付情報コピー` では、ZIP名、絶対パス、サイズ、SHA-256、送付文ファイルを1枚の照合値としてコピーできます。販売ページや注文管理画面へ添付する前に、`送付文コピー` と並べて使うことで誤添付を減らせます。
- `auto-note self-test` で、セットアップ、ランチャー健康チェック、クイック確認、アクションプラン、プライバシー監査、最新配布ZIP検証をまとめて確認できます。ランチャー健康チェックでは `auto-note-gui.bat`、隠しGUIランチャー、ショートカット、`auto-note safe display.lnk`、インストール/アンインストール補助ファイル、起動ログ/復旧導線を確認し、ショートカットで起動しない場合は `auto-note-gui.bat` を直接開く案内を出します。プライバシー監査NGが古い生成物由来のときは、`auto-note cleanup --project-dir . --privacy-failed --include-releases` でNG生成物だけを削除前に確認できます。クイック確認のWARNが記事チェック/レビューだけの場合はINFOに整理し、投稿前の磨き込みとして表示します。`--gui-smoke` を付けるとGUI初期化も確認し、`--report` を付けると `.auto-note\reports` にテキスト保存できます。
...
- `auto-note action-plan` で、準備度、クイック確認、投稿キュー、販売者情報の不足/公開URL警告、危険生成物候補から「いま優先すべき操作」を順位付きで表示できます。`--limit` で表示件数を絞っても `RC path / 残り作業` と `RC checkpoint / 今回の目途` は全体の残件をもとに出るため、短い表示でも販売RCまでの区切りを見失いません。プライバシーNGが古い生成物由来のときは、トップアクションから `危険生成物確認` と `auto-note cleanup --project-dir . --privacy-failed --include-releases` の削除前プレビューへ直接進めます。GUIのホームでは上位アクションを一覧し、投稿キューの対象記事や販売者情報確認へ直接移動、またはCLIコピーできます。
...
- `auto-note support` で、再現手順、直近の変更、添付物、匿名化済み診断プレビュー入りのサポート依頼Markdownを `.auto-note\support` に作成できます。通常作成時は送付前のプライバシー監査も表示します。`--bundle` を付けると、依頼文、診断ZIP、`SUPPORT_SEND_CHECKLIST.txt` を1つの問い合わせ一式ZIPにまとめ、manifest/checksum検証とプライバシー監査まで行います。`SUPPORT_SEND_CHECKLIST.txt` には送付判断、検証NG時は送らず作り直す案内、`.auto-note` や購入者ZIPを添付しない注意が入り、`--verify <zip>` で問い合わせ一式ZIPを検証できます。
- `auto-note privacy-audit` で、最新の診断ZIP、セルフテスト保存レポート、受入チェック保存レポート、販売準備レポート、販売者テンプレート、販売方針レビュー、販売ページ・納品最終レビューレポート、販売直前チェックリスト、販売確認記録、販売素材、販売一式ZIP、購入者向け単体ZIP、購入者向け送付文、購入者送付前チェックレポート、販売者向け納品記録、販売者送付前チェックリスト、改善プランレポート、運用サマリーレポート、予定ICS、投稿キューレポート、E2E確認レポート、問い合わせMarkdown、問い合わせ一式ZIP、配布ZIPに生のパス、ユーザー名、記事名、記事タイトル、メールアドレスが残っていないか確認できます。`--all` を付けると古い生成物も含めて監査します。
- `auto-note review` で記事のタイトル、概要、構成、導入、締め、タグ、画像、公開状態をスコア化し、次に直す項目を表示できます。
- `auto-note version` でバージョン、環境概要、インストール情報の状態、更新前バックアップの参照状態を表示できます。
...
- `scripts\smoke-install.ps1` で、インストール/アンインストール導線だけを一時フォルダで検証できます。`scripts\smoke-sales-delivery.ps1` では、クリーンな一時コピー上の販売一括作成、購入者向けZIP検証、送付前チェック保存、販売者向け送付記録作成、購入者送付の照合値付き販売直前チェックリスト保存、販売確認記録の保存と最新表示を確認できます。
...
- GitHub Actions の `CI` では、Windows上でPython構文チェック、全unittest、`auto-note quality --project-dir . --product-only`、隠しGUIランチャーのVBS構文チェック、`scripts\smoke-install.ps1` によるインストール/更新/アンインストールスモーク、出荷ZIPの作成と `auto-note release --verify`、`scripts\smoke-sales-delivery.ps1` による販売一括/購入者ZIP/送付前チェック/送付記録/販売確認記録スモーク、`auto-note gui --project-dir . --smoke` をpush/PRごとに実行します。
- `auto-note cleanup` で古い投稿ヘルパーHTML、診断ZIP、問い合わせ一式、記事CSV、セルフテスト保存レポート、受入チェック保存レポート、販売準備レポート、販売方針レビュー、販売ページ・納品最終レビューレポート、販売直前チェックリスト、販売確認記録、購入者向け送付文、購入者送付前チェックレポート、販売者向け納品記録、改善プランレポート、運用サマリーレポート、予定ICS、投稿キューレポート、E2E確認レポートをプレビューし、`--apply` で整理できます。配布ZIPを含める場合だけ `--include-releases` を付けます。`privacy-audit --all` で古い生成物がNGになった時は、`--privacy-failed` でNG生成物だけを確認できます。候補が多い場合も、個別一覧は先頭候補に抑え、種類別サマリーと残件数で全体を確認できます。
...
Googleログインなどで「このブラウザまたはアプリは安全ではない可能性があります」と表示される場合があるため、ダブルクリック版は普段使っている既定ブラウザを使います。迷った時はGUIのホーム/ヘルプ/コマンド検索にある `ログイン安全ガイド` を開き、`noteログイン` で普段のブラウザにログインしてから `投稿ヘルパー` で貼り付けてください。CLIだけで確認する場合は `auto-note login --default-browser` を使います。
...
auto-note sales-screenshots --project-dir .
auto-note sales-screenshots --project-dir . --verify ".auto-note\sales\screenshots\auto-note-sales-screenshots-YYYYMMDD-HHMMSS"
auto-note sales-listing --project-dir .
auto-note sales-listing --project-dir . --verify ".auto-note\sales\auto-note-sales-listing-kit-YYYYMMDD-HHMMSS.zip" --strict
...
今回の環境ではログインが弾かれやすいので、基本は手動ログイン対応モードを使ってください。GUIでは `ログイン安全ガイド`、CLIでは `auto-note login --default-browser` から普段のブラウザでログインし、投稿は `投稿ヘルパー` で転記します。
