...
from auto_note.gui import (
    ...
    _list_release_check_reports,
    _padding_with_vertical,
    _readable_vertical_padding,
    _release_check_report_path,
...
from auto_note.sales_launch import (
...
class ArticleTests(unittest.TestCase):
    ...
    def test_release_check_report_helpers(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = _release_check_report_path(project)
    ...
    def test_format_release_check_output_includes_execution_summary(self) -> None:
        ...
        finished = datetime(2026, 6, 10, 9, 1, 5)
        text = _format_release_check_output(
            status="OK",
            exit_code=0,
            started_at=started,
            finished_at=finished,
            command="scripts\\check-release.ps1 -Full",
            stdout="auto-note release check OK\n",
            stderr="",
        )
        self.assertIn("Release check / 販売前一括チェック", text)
        self.assertIn("status: OK", text)
        self.assertIn("exit_code: 0", text)
        self.assertIn("duration_seconds: 65.0", text)
        self.assertIn("auto-note release check OK", text)
    ...
    def test_home_buyer_send_evidence_summary_names_package_sha_and_matches(self) -> None:
        self.assertIn("ZIPなし", _home_buyer_send_evidence_summary(None, None, None))
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            package = root / "auto-note-buyer-delivery.zip"
            package.write_bytes(b"buyer package")
            package_sha = hashlib.sha256(package.read_bytes()).hexdigest()[:12]
            message = root / "buyer-delivery-message.txt"
            message.write_text(f"添付ZIP: {package.name}\nSHA-256: {package_sha}", encoding="utf-8")
            receipt = root / "seller-delivery-receipt.txt"
            receipt.write_text(f"Buyer delivery ZIP: {package.name}, SHA-256 {package_sha}", encoding="utf-8")

            ok = _home_buyer_send_evidence_summary(
                package,
                message,
                receipt,
                message_matches_package=True,
                receipt_matches_delivery=True,
            )
            self.assertIn("送付証跡:", ok)
            self.assertIn(package.name, ok)
            self.assertIn(package_sha, ok)
            self.assertIn("送付文一致", ok)
            self.assertIn("記録一致", ok)

            stale = _home_buyer_send_evidence_summary(
                package,
                message,
                receipt,
                message_matches_package=False,
            ...
            self.assertIn("送付文不一致", stale)
            self.assertIn("記録不一致", stale)

            ng = _home_buyer_send_evidence_summary(package, message, None, package_errors=["bad package"])
            self.assertIn("ZIP検証NG 1件", ng)
            self.assertIn("記録なし", ng)
...
"""
    ...
    def test_home_sales_screenshot_text_tracks_pack_verification(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp)
            self.assertEqual(_home_sales_screenshot_text([]), "なし")
            pack = create_sales_screenshot_pack(project)
            self.assertEqual(_home_sales_screenshot_text([pack.directory]), "あり")
            pack.html_path.unlink()
            self.assertEqual(_home_sales_screenshot_text([pack.directory]), "NG")

    def test_home_sales_artifact_text_names_missing_and_broken_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp)
            self.assertEqual(
                _home_sales_artifact_text([], [], [], []),
                "配布ZIP なし / 素材 なし / 掲載キット なし / 販売一式 なし",
            )
            release = project / "bad-release.zip"
            materials = project / "SALES_MATERIALS.md"
            listing = project / "bad-listing.zip"
            handoff = project / "bad-handoff.zip"
            release.write_text("not a zip", encoding="utf-8")
            materials.write_text("", encoding="utf-8")
            listing.write_text("not a zip", encoding="utf-8")
            handoff.write_text("not a zip", encoding="utf-8")

            self.assertEqual(
                _home_sales_artifact_text([release], [materials], [listing], [handoff]),
                "配布ZIP NG / 素材 NG / 掲載キット NG / 販売一式 NG",
            )

    def test_home_sales_artifact_ng_count_blocks_broken_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp)
            release = project / "bad-release.zip"
            materials = project / "SALES_MATERIALS.md"
            listing = project / "bad-listing.zip"
            handoff = project / "bad-handoff.zip"
            buyer = project / "bad-buyer.zip"
            release.write_text("not a zip", encoding="utf-8")
            materials.write_text("", encoding="utf-8")
            listing.write_text("not a zip", encoding="utf-8")
            handoff.write_text("not a zip", encoding="utf-8")
            buyer.write_text("not a zip", encoding="utf-8")
            screenshots = create_sales_screenshot_pack(project)
            screenshots.html_path.unlink()
    ...
    def test_home_sales_next_step_rebuilds_broken_release(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            release = Path(tmp) / "bad-release.zip"
            release.write_text("not a zip", encoding="utf-8")
        ...
        self.assertEqual(step.title, "配布ZIPを作り直す")
    ...
    def test_backup_and_diagnostics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(report) as archive:
                ...
                self.assertIn("sales-launch.txt", names)
                ...
                sales_launch_text = archive.read("sales-launch.txt").decode("utf-8")
            ...
            self.assertNotIn(str(project), sales_launch_text)
            ...
            self.assertIn("Sales launch checklist / 販売直前チェック", sales_launch_text)
            self.assertIn("No sales launch checklists found", sales_launch_text)
            ...
            self.assertIn("sales_launch_checklists:", maintenance_text)
            ...
            self.assertNotIn("バックアップ記事", sales_launch_text)
    ...
    def test_acceptance_check_summarizes_buyer_handoff_and_cli(self) -> None:
        ...
        self.assertIn("起動OK", full_output)
        self.assertIn("操作", full_output)
        self.assertNotIn("home_quick_action_items", full_output)
        self.assertNotIn("GUI smoke OK:", full_output)
        self.assertNotIn("startup OK", full_output)
        self.assertNotIn("quick actions", full_output)
        self.assertIn("表示の読みやすさ", full_output)
        self.assertIn("可読性OK", full_output)
        self.assertIn("ボタン文字OK", full_output)
        self.assertNotIn("readability OK", full_output)
        self.assertNotIn("button labels OK", full_output)
    ...
    def test_sales_handoff_packages_release_and_seller_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(result.path) as archive:
                ...
                handoff_screenshot_captions = archive.read("sales_screenshots/SCREENSHOT_CAPTIONS.md").decode("utf-8")
                handoff_screenshot_preview = archive.read("sales_screenshots/index.html").decode("utf-8")
            ...
            screenshot_packs = list_sales_screenshot_packs(project)
        ...
        self.assertIn("sales_screenshots/SCREENSHOT_CAPTIONS.md", names)
        self.assertIn("sales_screenshots/index.html", names)
        self.assertIn("sales_screenshots/README.txt", names)
        self.assertIn("sales_screenshots/01-home-readiness.svg", names)
        ...
        self.assertIn("sales_screenshots/", delivery_checklist)
        ...
        self.assertIn("sales_screenshots/index.html", seller_checklist)
        ...
        self.assertIn("ホームで準備度", handoff_screenshot_captions)
        self.assertIn("auto-note sales screenshots", handoff_screenshot_preview)
        ...
        self.assertGreaterEqual(len(screenshot_packs), 1)
    ...
    def test_sales_finalize_creates_and_verifies_buyer_delivery(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            latest_sales_launch_confirmation_path, latest_sales_launch_confirmation_text = (
                find_latest_sales_launch_confirmation(project)
            )
            ...
            with redirect_stdout(sales_launch_latest_confirmation_cli_output):
                sales_launch_latest_confirmation_cli_code = cli_main(
                    ["sales-launch", "--project-dir", str(project), "--latest-confirmation"]
                )
            ...
            privacy_after_sales_launch_text = format_privacy_audit_report(privacy_after_sales_launch)
            newer_release_path = create_release_package(project)
            newer_release_time = release_path.stat().st_mtime + 60
            os.utime(newer_release_path, (newer_release_time, newer_release_time))
            stale_buyer_send_readiness = run_buyer_send_readiness(project)
            stale_buyer_send_readiness_text = format_buyer_send_readiness_report(stale_buyer_send_readiness)
        ...
        self.assertEqual(latest_sales_launch_confirmation_path, sales_launch_confirmation_path)
...
""",
    ...
    def test_quality_checks_workflow_values(self) -> None:
        ...
        self.assertIn("sales handoff includes sales screenshots:fail", product_details)
        self.assertIn("sales handoff verifies sales screenshots:fail", product_details)
        self.assertIn("sales handoff checksums sales screenshots:fail", product_details)
        ...
        self.assertIn("GUI home sales timeline panel:fail", product_details)
        ...
        self.assertIn("GUI no fixed 9pt status labels:fail", product_details)
        ...
        self.assertIn("README current readable Japanese font guidance:fail", product_details)
        self.assertIn("README actual display font diagnostics guidance:fail", product_details)
        self.assertIn("changelog current readable Japanese font guidance:fail", product_details)
        ...
        self.assertIn("GUI smoke home sales includes buyer send evidence:fail", product_details)
        ...
        self.assertIn("GUI buyer support request open action:fail", product_details)
        self.assertIn("GUI buyer support request command:fail", product_details)
        self.assertIn("GUI buyer delivery ZIP path copy action:fail", product_details)
        self.assertIn("GUI buyer delivery ZIP location action:fail", product_details)
        self.assertIn("GUI buyer delivery ZIP location guard:fail", product_details)
        self.assertIn("GUI buyer delivery ZIP location feedback:fail", product_details)
        self.assertIn("GUI buyer delivery ZIP path copy clipboard:fail", product_details)
        self.assertIn("GUI buyer delivery ZIP path copy guard:fail", product_details)
        self.assertIn("GUI buyer delivery sheet copy action:fail", product_details)
        self.assertIn("GUI buyer delivery sheet copy contents:fail", product_details)
        self.assertIn("GUI buyer delivery sheet copy clipboard:fail", product_details)
        self.assertIn("GUI buyer delivery sheet copy guard:fail", product_details)
        ...
        self.assertIn("GUI sales finalize opens buyer support request:fail", product_details)
        ...
        self.assertIn("GUI full release check command palette:fail", product_details)
        ...
        self.assertIn("GUI home buyer send evidence row:fail", product_details)
        self.assertIn("GUI home buyer send evidence helper:fail", product_details)
        ...
        self.assertIn("README home buyer send evidence guidance:fail", product_details)
        ...
        self.assertIn("README sales launch guidance:fail", product_details)
        self.assertIn("README sales launch report guidance:fail", product_details)
        self.assertIn("README GUI full release check guidance:fail", product_details)
        self.assertIn("README release check report guidance:fail", product_details)
        ...
        self.assertIn("README home sales timeline guidance:fail", product_details)
        ...
        self.assertIn("product readiness home buyer send evidence guidance:fail", product_details)
        ...
        self.assertIn("product readiness release check report artifact:fail", product_details)
        ...
        self.assertIn("product readiness sales launch command:fail", product_details)
        ...
        self.assertIn("sales handoff includes sales screenshots:pass", launcher_details)
        self.assertIn("sales handoff verifies sales screenshots:pass", launcher_details)
        self.assertIn("sales handoff checksums sales screenshots:pass", launcher_details)
        ...
        self.assertIn("GUI home sales timeline panel:pass", launcher_details)
        ...
        self.assertIn("GUI no fixed 9pt status labels:pass", launcher_details)
        ...
        self.assertIn("README current readable Japanese font guidance:pass", launcher_details)
        self.assertIn("README actual display font diagnostics guidance:pass", launcher_details)
        self.assertIn("changelog current readable Japanese font guidance:pass", launcher_details)
        ...
        self.assertIn("GUI smoke home sales includes buyer send evidence:pass", launcher_details)
        ...
        self.assertIn("GUI buyer support request open action:pass", launcher_details)
        self.assertIn("GUI buyer support request command:pass", launcher_details)
        self.assertIn("GUI buyer delivery ZIP path copy action:pass", launcher_details)
        self.assertIn("GUI buyer delivery ZIP location action:pass", launcher_details)
        self.assertIn("GUI buyer delivery ZIP location guard:pass", launcher_details)
        self.assertIn("GUI buyer delivery ZIP location feedback:pass", launcher_details)
        self.assertIn("GUI buyer delivery ZIP path copy clipboard:pass", launcher_details)
        self.assertIn("GUI buyer delivery ZIP path copy guard:pass", launcher_details)
        self.assertIn("GUI buyer delivery sheet copy action:pass", launcher_details)
        self.assertIn("GUI buyer delivery sheet copy contents:pass", launcher_details)
        self.assertIn("GUI buyer delivery sheet copy clipboard:pass", launcher_details)
        self.assertIn("GUI buyer delivery sheet copy guard:pass", launcher_details)
        ...
        self.assertIn("GUI sales finalize opens buyer support request:pass", launcher_details)
        ...
        self.assertIn("GUI sales launch confirmation copy feedback:pass", launcher_details)
        ...
        self.assertIn("GUI full release check command palette:pass", launcher_details)
        ...
        self.assertIn("GUI home buyer send evidence row:pass", launcher_details)
        self.assertIn("GUI home buyer send evidence helper:pass", launcher_details)
        ...
        self.assertIn("README home buyer send evidence guidance:pass", launcher_details)
        ...
        self.assertIn("README sales launch confirmation copy guidance:pass", launcher_details)
        ...
        self.assertIn("README home sales timeline guidance:pass", launcher_details)
        ...
        self.assertIn("product readiness home buyer send evidence guidance:pass", launcher_details)
        ...
        self.assertIn("product readiness release check report artifact:pass", launcher_details)
